...
contract Comptroller is ComptrollerV5Storage, ComptrollerInterface, ComptrollerErrorReporter, ExponentialNoError {
    ...
    event LiquidatorWhiteListOnUpdated(bool isOn);
    ...
    function _setLiquidatorWhiteListOn(bool isOn) external {
        require(msg.sender == admin, "Only admin can call this");
        liquidatorWhiteListOn = isOn;
        emit LiquidatorWhiteListOnUpdated(isOn);
    }

    function _addLiquidator(address liquidator) external {
        require(msg.sender == admin, "Only admin can call this");
        require(liquidator != address(0), "liquidator is zero address");
        liquidatorWhiteList[liquidator] = true;
        emit LiquidatorAdded(liquidator);
    }

    function _removeLiquidator(address liquidator) external {
        require(msg.sender == admin, "Only admin can call this");
        require(liquidator != address(0), "liquidator is zero address");
        require(liquidatorWhiteList[liquidator] == true, "liquidator not in");
        liquidatorWhiteList[liquidator] = false;
